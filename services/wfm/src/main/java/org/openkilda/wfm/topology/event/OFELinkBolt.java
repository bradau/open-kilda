...
public class OFELinkBolt
    ...
    public OFELinkBolt(TopologyConfig config) {
        ...
        islHealthCheckInterval =
                Preconditions.checkNotNull(config.getDiscoveryInterval(), "DiscoveryInterval is null");
        Preconditions.checkArgument(islHealthCheckInterval > 0,
                "Invalid value for DiscoveryInterval: %s", islHealthCheckInterval);
        islHealthCheckTimeout =
                Preconditions.checkNotNull(config.getDiscoveryTimeout(), "DiscoveryTimeout is null");
        Preconditions.checkArgument(islHealthCheckTimeout > 0,
                "Invalid value for DiscoveryTimeout: %s", islHealthCheckTimeout);
        this.islHealthFailureLimit =
                Preconditions.checkNotNull(config.getDiscoveryLimit(), "DiscoveryLimit is null");
