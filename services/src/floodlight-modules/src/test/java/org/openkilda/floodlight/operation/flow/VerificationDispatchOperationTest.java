...
public class VerificationDispatchOperationTest extends AbstractVerificationOperationTest {
    @Test
    public void skipIfInsufficientCapabilities() {
        ...
        VerificationDispatchOperation subject = new VerificationDispatchOperation(operationContext, request);

        List<Optional<Operation>> subOperations = subject.produceSubOperations();
        verify(destSwitch);

        Assert.assertEquals("Produced sub operations list is not empty",0, subOperations.size());
