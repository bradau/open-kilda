...
public class OFBatchService extends AbstractOfHandler implements IFloodlightService {
    ...
    public synchronized void push(Operation initiator, List<OFPendingMessage> payload) throws OFInstallException {
    ...
    public boolean handle(IOFSwitch sw, OFMessage message, FloodlightContext context) {
        ...
        if (completed != null) {
            log.debug("Send complete signal to pending operation: {}", completed.operation);
    ...
    private class Task {
        ...
        Task(Operation operation, BatchRecord batch) {
